...
function TallerInterno() {
  ...
  const tituloDe = (t) => t.titulo || [t.vehiculos?.patente, t.vehiculos?.marca, t.vehiculos?.modelo, [t.clientes?.nombre, t.clientes?.apellidos].filter(Boolean).join(' '), t.ot_numero ? 'OT ' + t.ot_numero : ''].filter(Boolean).join(' ')
    ...
    <Modal abierto titulo={tituloDe(t)} onClose={onClose} ancho="max-w-3xl">
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="space-y-2">
            {presups.map((p) => (
              <PresupuestoTallerCard key={p.id} p={p} t={t} esJefe={esJefe} esCompras={esCompras} perfil={perfil} editable={false}
